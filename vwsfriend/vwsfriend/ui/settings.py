...
from flask import Blueprint, render_template, current_app, abort, flash, redirect, url_for, request
...
class HybridVehicleSettingsForm(VehicleSettingsForm):
    ...
    secondary_capacity = IntegerField('Usable Battery Capacity (net) in kWh', validators=[Optional(), NumberRange(min=1, max=500)], filters = [lambda x: x or None])
...
def vehicleDBParameters(vin):
    ...
    return render_template('settings/vehicledbparameters.html', vehicle=foundVehicle, form=form)

@bp.route('/vehicle/abrp/<vin>', methods=['GET', 'POST'])
def vehicleABRPSettings(vin):
    if vin not in current_app.weConnect.vehicles:
        abort(404, f"Vehicle with VIN {vin} doesn't exist.")
    foundVehicle = current_app.weConnect.vehicles[vin]
    agent = None
    ...
    return render_template('settings/abrpsettings.html', vehicle=foundVehicle, form=form)
